...
export default function Rappels() {
  ...
  const joursRappelInactivite = salon && 'joursRappelInactivite' in salon ? salon.joursRappelInactivite : 30;
  const joursRappelSuivi = salon && 'joursRappelSuivi' in salon ? salon.joursRappelSuivi : 14;
  const configFidelite = salon && 'configFidelite' in salon ? salon.configFidelite : null;

  const clientesInactives = getInactiveClients(joursRappelInactivite);
  ...
  const clientesSuivi = useMemo(() => clients.filter(c => {
    ...
    return daysSince >= 7 && daysSince <= joursRappelSuivi;
  }), [clients, joursRappelSuivi]);

  const clientesProcheCadeau = useMemo(() => clients.filter(c => {
    if (!configFidelite) return false;
    const remaining = configFidelite.visitesRequises - (c.pointsFidelite % configFidelite.visitesRequises);
    return remaining <= 2 && remaining > 0 && c.pointsFidelite > 0;
  }), [clients, configFidelite]);
          ...
          <TabsContent value="suivi" className="m-0 p-5 sm:p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground font-semibold">{t('reminders.satisfactionDesc', 'Clientes venues entre 7 et {days} jours - demandez leur avis !', { days: String(salon?.joursRappelSuivi || 14) })}</p>
              </div>
            </div>
...
console.log('Rappels.tsx refactored with clean UI, pagination, search, and full localization');
