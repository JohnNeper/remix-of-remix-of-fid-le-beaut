  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-1 min-h-0 overflow-hidden text-left font-sans">
        ...
        <div className="shrink-0 bg-background/95 dark:bg-slate-900/95 backdrop-blur-md p-4 sm:p-5 border-t border-slate-200 dark:border-slate-800 z-10 flex items-center gap-3">
          <Button type="button" variant="outline" onClick={onCancel} className="flex-1 h-11 rounded-2xl font-extrabold text-xs text-foreground dark:text-slate-100 bg-muted/60 dark:bg-slate-800 hover:bg-muted dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700">
            {t('common.cancel') || 'Annuler'}
          </Button>
          <Button
            type="submit"
            disabled={!dayConfig || !dayConfig.open}
            className="flex-1 h-11 rounded-2xl font-extrabold text-xs bg-rose-600 hover:bg-rose-700 text-white shadow-md shadow-rose-600/30 border-0 disabled:opacity-50"
          >
